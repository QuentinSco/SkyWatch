const REGION_LABELS = {
    AMN:'🌎 Amérique du Nord', AMS:'🌎 Amérique du Sud',
    AMO:'🏝 Amérique / Outremer', EUR:'🌍 Europe',
  ...
  export function renderRegion(region, alerts) {
    ...
    const borderColor = filtered.some(a=>a.severity==='red') ? 'border-red-500'
      : filtered.some(a=>a.severity==='orange') ? 'border-orange-400' : 'border-yellow-400';
            ...
            <tbody>
              ${filtered.map((a,i) => `
                <tr class="border-b border-gray-100 hover:bg-blue-50 transition ${i%2===0?'bg-white':'bg-gray-50/50'}">
                  ...
                  <td class="py-2 px-4 text-gray-500 text-xs truncate-cell"
